...
class Fly:
    """Flocking boid entity driven by an FSM with Flock, Fleeing, and Idle states."""
    ...
    def update(self, dt, flies, frog, bounds_rect, bubbles, vfx=None):
        """Update FSM transitions and state behaviors."""
        BubbleFleeRange = 140.0
        StopFleeingRange = 220.0
        IdleDistance = 380.0
        IdleDelay    = 3.0
        ...
        # Soft containment inside arena bounds
    ...
    def draw(self, surf):
        ...
        shade_factor = 0.85 + (self.depth_scale - 0.85) * 0.5
        color = (
    ...
    def draw_debug(self, surf, flies, font):
        """Render debug overlay visualization for fly entity."""
        for f in flies:
            if f is not self:
