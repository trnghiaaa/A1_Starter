...
class Bubble:
    """Projectile fired by the frog that pops after lifetime or impact."""
...
class Frog:
    """Player agent governed by Arrive steering and bubble projectile shooting."""
    ...
    def set_target(self, p):
        """Set movement destination for Arrive steering."""
        self.target = V2(p)

    def shoot(self):
        """Spawn a bubble projectile along current facing direction."""
    ...
    def start_hurt(self):
        """Trigger temporary invulnerability timer after damage."""
        if self.hurt_timer <= 0:
            self.hurt_timer = HURT_INVULN

    def can_be_hurt(self):
        """Return True if invulnerability window has expired."""
    ...
    def draw_debug(self, surf, font):
        """Render debug overlay visualization for Frog."""
        tx, ty = int(self.target.x), int(self.target.y)
